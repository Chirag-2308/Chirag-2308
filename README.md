## Hey, I'm Chirag Singh Rothan 👋

By day I wrangle data and build AI that keeps machines from breaking.
By night I learn to break things — ethically, I promise. 🔐
Always curious. Always building. Occasionally sleeping.

> *"I believe the best systems are both intelligent and secure — built by people who never stop asking why."*

---

### 🏭 What I'm Building
**Predictive Maintenance System — Industrial AI**

Machines fail. Factories lose money. I'm building an ML system that sees failure coming *before it happens* — using Python, SQL, and smart data pipelines. Think of it as a doctor for industrial equipment.

---

### 🔐 What I'm Obsessing Over
**Cybersecurity — thinking like an attacker**

Diving deep into ethical hacking, network security, and vulnerability assessment. My goal? Build data systems that aren't just smart — but ones that attackers simply can't touch.

---

### 🤝 Let's Build Something Together
**Open Source · Data Science Research · AI Security**

I'm on the lookout for collaborators excited about:
- 📊 Open source **Data Science** tools
- 🛡️ **Cybersecurity research** — especially where AI meets security
- 🤖 **Industrial AI + IoT security** projects

I show up with Python, SQL, Linux, curiosity, and a thermos of chai. ☕
If your project solves a real problem and you need someone who genuinely cares — **let's talk.**

---

### 🛠️ Tech Stack

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Cybersecurity](https://img.shields.io/badge/Cybersecurity-CC0000?style=flat&logo=hackthebox&logoColor=white)

---

### 🙋 A Few Things About Me

- 🔍 I debug data the way detectives crack cases — follow the trail, trust nothing
- 🔐 Breaking systems (legally) is my hobby. Security through curiosity
- 🤖 Teaching machines to predict the unpredictable — one model at a time
- 📊 If it has a pattern, I will find it. If it doesn't, I'll look harder
- ☕ Runs on Python and strong chai. In that order
- 📖 Always learning. Always building. Never done

---

### 📬 Find Me on the Internet

I don't bite. Reach out if you want to collaborate, talk data, or geek out about security.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Chirag%20Singh%20Rothan-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/chirag-singh-rothan-617a48314)
[![GitHub](https://img.shields.io/badge/GitHub-Chirag--2308-181717?style=flat&logo=github&logoColor=white)](https://github.com/Chirag-2308)

---

### 💬 Ask Me About
`Data Science` `Machine Learning` `Cybersecurity` `Python` `SQL` `Linux` `Industrial AI` `Predictive Maintenance`

---

> *"Debugging machines by day, breaking systems by night — powered by Python and strong chai."* ☕
